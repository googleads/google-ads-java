...
import com.google.ads.googleads.v23.services.AudienceInsightsServiceGrpc.AudienceInsightsServiceImplBase;
...
public class MockAudienceInsightsServiceImpl extends AudienceInsightsServiceImplBase {
  ...
  @Override
  public void generateAudienceDefinition(
      GenerateAudienceDefinitionRequest request,
      StreamObserver<GenerateAudienceDefinitionResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof GenerateAudienceDefinitionResponse) {
      requests.add(request);
      responseObserver.onNext(((GenerateAudienceDefinitionResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method GenerateAudienceDefinition, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
  ...
  @Override
  public void generateTargetingSuggestionMetrics(
      GenerateTargetingSuggestionMetricsRequest request,
      StreamObserver<GenerateTargetingSuggestionMetricsResponse> responseObserver) {
    Object response = responses.poll();
    if (response instanceof GenerateTargetingSuggestionMetricsResponse) {
      requests.add(request);
      responseObserver.onNext(((GenerateTargetingSuggestionMetricsResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method GenerateTargetingSuggestionMetrics, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
