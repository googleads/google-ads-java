...
import com.google.ads.googleads.v7.services.stub.AdGroupServiceStub;
import com.google.ads.googleads.v7.services.stub.AdGroupServiceStubSettings;
...
public class AdGroupServiceClient implements BackgroundResource {
   ...
   * <pre>{@code
   * try (AdGroupServiceClient adGroupServiceClient = AdGroupServiceClient.create()) {
   *   String resourceName = AdGroupName.of("[CUSTOMER_ID]", "[AD_GROUP_ID]").toString();
   *   AdGroup response = adGroupServiceClient.getAdGroup(resourceName);
   * }
   * }</pre>
   *
