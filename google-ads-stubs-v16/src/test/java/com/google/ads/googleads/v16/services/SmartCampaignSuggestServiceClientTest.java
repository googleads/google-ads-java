...
import com.google.ads.googleads.v16.common.SmartCampaignAdInfo;
...
public class SmartCampaignSuggestServiceClientTest {
  ...
  public void suggestKeywordThemesTest() throws Exception {
    ...
    SuggestKeywordThemesResponse actualResponse = client.suggestKeywordThemes(request);
    Assert.assertEquals(expectedResponse, actualResponse);
    ...
    try {
      SuggestKeywordThemesRequest request =
          SuggestKeywordThemesRequest.newBuilder()
              ...
              .setSuggestionInfo(SmartCampaignSuggestionInfo.newBuilder().build())
              .build();
      client.suggestKeywordThemes(request);
      Assert.fail("No exception raised");
    } catch (InvalidArgumentException e) {
      // Expected exception.
    }
