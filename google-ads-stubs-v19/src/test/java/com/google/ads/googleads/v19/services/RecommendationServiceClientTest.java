...
import com.google.ads.googleads.v19.enums.AdvertisingChannelTypeEnum;
import com.google.ads.googleads.v19.enums.RecommendationTypeEnum;
import com.google.ads.googleads.v19.resources.Recommendation;
...
public class RecommendationServiceClientTest {
  ...
  @Test
  public void generateRecommendationsTest() throws Exception {
    GenerateRecommendationsResponse expectedResponse =
        GenerateRecommendationsResponse.newBuilder()
            .addAllRecommendations(new ArrayList<Recommendation>())
            .build();
    mockRecommendationService.addResponse(expectedResponse);
    ...
    GenerateRecommendationsResponse actualResponse =
        client.generateRecommendations(customerId, recommendationTypes, advertisingChannelType);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<AbstractMessage> actualRequests = mockRecommendationService.getRequests();
    Assert.assertEquals(1, actualRequests.size());
    GenerateRecommendationsRequest actualRequest =
        ((GenerateRecommendationsRequest) actualRequests.get(0));

    Assert.assertEquals(customerId, actualRequest.getCustomerId());
    Assert.assertEquals(recommendationTypes, actualRequest.getRecommendationTypesList());
    Assert.assertEquals(advertisingChannelType, actualRequest.getAdvertisingChannelType());
    Assert.assertTrue(
        channelProvider.isHeaderSent(
            ApiClientHeaderProvider.getDefaultApiClientHeaderKey(),
            GaxGrpcProperties.getDefaultApiClientHeaderPattern()));
  ...
  @Test
  public void generateRecommendationsExceptionTest() throws Exception {
    StatusRuntimeException exception = new StatusRuntimeException(io.grpc.Status.INVALID_ARGUMENT);
    mockRecommendationService.addException(exception);
