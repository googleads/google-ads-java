...
import com.google.ads.googleads.v1.resources.LocationView;
...
public class LocationViewServiceClientTest {
  ...
  public void getLocationViewTest() {
    String resourceName2 = "resourceName2625949903";
    LocationView expectedResponse =
        LocationView.newBuilder().setResourceName(resourceName2).build();
    mockLocationViewService.addResponse(expectedResponse);

    String formattedResourceName =
        LocationViewServiceClient.formatLocationViewName("[CUSTOMER]", "[LOCATION_VIEW]");

    LocationView actualResponse = client.getLocationView(formattedResourceName);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<GeneratedMessageV3> actualRequests = mockLocationViewService.getRequests();
