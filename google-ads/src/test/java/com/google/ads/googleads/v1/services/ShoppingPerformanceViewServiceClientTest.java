...
public class ShoppingPerformanceViewServiceClientTest {
  ...
  public void getShoppingPerformanceViewTest() {
    ...
    ShoppingPerformanceView expectedResponse =
        ShoppingPerformanceView.newBuilder().setResourceName(resourceName2).build();
    mockShoppingPerformanceViewService.addResponse(expectedResponse);

    String formattedResourceName =
        ShoppingPerformanceViewServiceClient.formatShoppingPerformanceViewName(
            "[CUSTOMER]", "[SHOPPING_PERFORMANCE_VIEW]");

    ShoppingPerformanceView actualResponse =
        client.getShoppingPerformanceView(formattedResourceName);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<GeneratedMessageV3> actualRequests = mockShoppingPerformanceViewService.getRequests();
