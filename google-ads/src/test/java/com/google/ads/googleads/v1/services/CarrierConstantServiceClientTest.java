...
public class CarrierConstantServiceClientTest {
  ...
  public void getCarrierConstantTest() {
    String resourceName2 = "resourceName2625949903";
    CarrierConstant expectedResponse =
        CarrierConstant.newBuilder().setResourceName(resourceName2).build();
    mockCarrierConstantService.addResponse(expectedResponse);

    String formattedResourceName =
        CarrierConstantServiceClient.formatCarrierConstantName("[CARRIER_CONSTANT]");

    CarrierConstant actualResponse = client.getCarrierConstant(formattedResourceName);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<GeneratedMessageV3> actualRequests = mockCarrierConstantService.getRequests();
