...
import com.google.ads.googleads.v2.resources.TopicConstant;
...
public class TopicConstantServiceClientTest {
  ...
  public void getTopicConstantTest() {
    ...
    TopicConstant expectedResponse =
        TopicConstant.newBuilder().setResourceName(resourceName2).build();
    mockTopicConstantService.addResponse(expectedResponse);

    String formattedResourceName =
        TopicConstantServiceClient.formatTopicConstantName("[TOPIC_CONSTANT]");

    TopicConstant actualResponse = client.getTopicConstant(formattedResourceName);
