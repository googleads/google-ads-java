...
public class DistanceViewServiceClientTest {
  ...
  public void getDistanceViewTest() {
    String resourceName2 = "resourceName2625949903";
    DistanceView expectedResponse =
        DistanceView.newBuilder().setResourceName(resourceName2).build();
    mockDistanceViewService.addResponse(expectedResponse);

    String formattedResourceName =
        DistanceViewServiceClient.formatDistanceViewName("[CUSTOMER]", "[DISTANCE_VIEW]");

    DistanceView actualResponse = client.getDistanceView(formattedResourceName);
