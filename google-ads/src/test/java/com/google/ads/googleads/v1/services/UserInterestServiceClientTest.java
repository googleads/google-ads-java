...
public class UserInterestServiceClientTest {
  ...
  public void getUserInterestTest() {
    ...
    UserInterest expectedResponse =
        UserInterest.newBuilder().setResourceName(resourceName2).build();
    mockUserInterestService.addResponse(expectedResponse);

    String formattedResourceName =
        UserInterestServiceClient.formatUserInterestName("[CUSTOMER]", "[USER_INTEREST]");

    UserInterest actualResponse = client.getUserInterest(formattedResourceName);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<GeneratedMessageV3> actualRequests = mockUserInterestService.getRequests();
