...
public class HotelGroupViewServiceClientTest {
  ...
  public void getHotelGroupViewTest() {
    ...
    HotelGroupView expectedResponse =
        HotelGroupView.newBuilder().setResourceName(resourceName2).build();
    mockHotelGroupViewService.addResponse(expectedResponse);

    String formattedResourceName =
        HotelGroupViewServiceClient.formatHotelGroupViewName("[CUSTOMER]", "[HOTEL_GROUP_VIEW]");

    HotelGroupView actualResponse = client.getHotelGroupView(formattedResourceName);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<GeneratedMessageV3> actualRequests = mockHotelGroupViewService.getRequests();
