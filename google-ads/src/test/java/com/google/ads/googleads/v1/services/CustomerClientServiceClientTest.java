...
public class CustomerClientServiceClientTest {
  ...
  public void getCustomerClientTest() {
    String resourceName2 = "resourceName2625949903";
    CustomerClient expectedResponse =
        CustomerClient.newBuilder().setResourceName(resourceName2).build();
    mockCustomerClientService.addResponse(expectedResponse);

    String formattedResourceName =
        CustomerClientServiceClient.formatCustomerClientName("[CUSTOMER]", "[CUSTOMER_CLIENT]");

    CustomerClient actualResponse = client.getCustomerClient(formattedResourceName);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<GeneratedMessageV3> actualRequests = mockCustomerClientService.getRequests();
