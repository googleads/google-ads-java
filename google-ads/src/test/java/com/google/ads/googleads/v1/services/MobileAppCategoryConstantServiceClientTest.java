...
public class MobileAppCategoryConstantServiceClientTest {
  ...
  public void getMobileAppCategoryConstantTest() {
    ...
    MobileAppCategoryConstant expectedResponse =
        MobileAppCategoryConstant.newBuilder().setResourceName(resourceName2).build();
    mockMobileAppCategoryConstantService.addResponse(expectedResponse);

    String formattedResourceName =
        MobileAppCategoryConstantServiceClient.formatMobileAppCategoryConstantName(
            "[MOBILE_APP_CATEGORY_CONSTANT]");

    MobileAppCategoryConstant actualResponse =
        client.getMobileAppCategoryConstant(formattedResourceName);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<GeneratedMessageV3> actualRequests = mockMobileAppCategoryConstantService.getRequests();
