...
public class ChangeStatusServiceClientTest {
  ...
  public void getChangeStatusTest() {
    ...
    ChangeStatus expectedResponse =
        ChangeStatus.newBuilder().setResourceName(resourceName2).build();
    mockChangeStatusService.addResponse(expectedResponse);

    String formattedResourceName =
        ChangeStatusServiceClient.formatChangeStatusName("[CUSTOMER]", "[CHANGE_STATUS]");

    ChangeStatus actualResponse = client.getChangeStatus(formattedResourceName);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<GeneratedMessageV3> actualRequests = mockChangeStatusService.getRequests();
