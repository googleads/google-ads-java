...
public class MobileDeviceConstantServiceClientTest {
  ...
  public void getMobileDeviceConstantTest() {
    ...
    MobileDeviceConstant expectedResponse =
        MobileDeviceConstant.newBuilder().setResourceName(resourceName2).build();
    mockMobileDeviceConstantService.addResponse(expectedResponse);

    String formattedResourceName =
        MobileDeviceConstantServiceClient.formatMobileDeviceConstantName(
            "[MOBILE_DEVICE_CONSTANT]");

    MobileDeviceConstant actualResponse = client.getMobileDeviceConstant(formattedResourceName);
