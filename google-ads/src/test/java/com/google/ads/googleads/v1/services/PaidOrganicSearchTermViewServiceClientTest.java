...
public class PaidOrganicSearchTermViewServiceClientTest {
  ...
  public void getPaidOrganicSearchTermViewTest() {
    String resourceName2 = "resourceName2625949903";
    PaidOrganicSearchTermView expectedResponse =
        PaidOrganicSearchTermView.newBuilder().setResourceName(resourceName2).build();
    mockPaidOrganicSearchTermViewService.addResponse(expectedResponse);

    String formattedResourceName =
        PaidOrganicSearchTermViewServiceClient.formatPaidOrganicSearchTermViewName(
            "[CUSTOMER]", "[PAID_ORGANIC_SEARCH_TERM_VIEW]");

    PaidOrganicSearchTermView actualResponse =
        client.getPaidOrganicSearchTermView(formattedResourceName);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<GeneratedMessageV3> actualRequests = mockPaidOrganicSearchTermViewService.getRequests();
