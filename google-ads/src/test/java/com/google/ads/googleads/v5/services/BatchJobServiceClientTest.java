...
public class BatchJobServiceClientTest {
  ...
  public void getBatchJobTest() {
    com.google.ads.googleads.v5.services.BatchJobName resourceName2 =
        com.google.ads.googleads.v5.services.BatchJobName.of("[CUSTOMER]", "[BATCH_JOB]");
    BatchJob expectedResponse =
        BatchJob.newBuilder().setResourceName(resourceName2.toString()).build();
    mockBatchJobService.addResponse(expectedResponse);

    BatchJobName resourceName = BatchJobName.of("[CUSTOMER]", "[BATCH_JOB]");

    BatchJob actualResponse = client.getBatchJob(resourceName);
    Assert.assertEquals(expectedResponse, actualResponse);
    ...
    Assert.assertEquals(resourceName, BatchJobName.parse(actualRequest.getResourceName()));
    Assert.assertTrue(
        channelProvider.isHeaderSent(
            ApiClientHeaderProvider.getDefaultApiClientHeaderKey(),
            GaxGrpcProperties.getDefaultApiClientHeaderPattern()));
  ...
  public void getBatchJobExceptionTest() throws Exception {
    ...
    try {
      ...
      client.getBatchJob(resourceName);
      Assert.fail("No exception raised");
    } catch (InvalidArgumentException e) {
      // Expected exception
    }
  ...
  public void runBatchJobTest() throws Exception {
    ...
    Empty actualResponse = client.runBatchJobAsync(resourceName).get();
    Assert.assertEquals(expectedResponse, actualResponse);
  ...
  @Test
  @SuppressWarnings("all")
  public void addBatchJobOperationsTest2() {
    long totalOperations = 1685086151L;
    String nextSequenceToken = "nextSequenceToken388644551";
    AddBatchJobOperationsResponse expectedResponse =
        AddBatchJobOperationsResponse.newBuilder()
