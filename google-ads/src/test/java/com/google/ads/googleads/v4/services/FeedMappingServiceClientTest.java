...
public class FeedMappingServiceClientTest {
  ...
  public void getFeedMappingTest() {
    ...
    FeedMapping expectedResponse = FeedMapping.newBuilder().setResourceName(resourceName2).build();
    mockFeedMappingService.addResponse(expectedResponse);

    String formattedResourceName =
        FeedMappingServiceClient.formatFeedMappingName("[CUSTOMER]", "[FEED_MAPPING]");
