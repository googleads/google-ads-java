...
public class GroupPlacementViewServiceClientTest {
  ...
  public void getGroupPlacementViewTest() {
    ...
    GroupPlacementView expectedResponse =
        GroupPlacementView.newBuilder().setResourceName(resourceName2).build();
    mockGroupPlacementViewService.addResponse(expectedResponse);

    String formattedResourceName =
        GroupPlacementViewServiceClient.formatGroupPlacementViewName(
            "[CUSTOMER]", "[GROUP_PLACEMENT_VIEW]");

    GroupPlacementView actualResponse = client.getGroupPlacementView(formattedResourceName);
