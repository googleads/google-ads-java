// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: google/ads/googleads/v5/services/keyword_plan_idea_service.proto

package com.google.ads.googleads.v5.services;

public interface SiteSeedOrBuilder extends
    // @@protoc_insertion_point(interface_extends:google.ads.googleads.v5.services.SiteSeed)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <pre>
   * The domain name of the site. If the customer requesting the ideas doesn't
   * own the site provided only public information is returned.
   * </pre>
   *
   * <code>.google.protobuf.StringValue site = 1;</code>
   * @return Whether the site field is set.
   */
  boolean hasSite();
  /**
   * <pre>
   * The domain name of the site. If the customer requesting the ideas doesn't
   * own the site provided only public information is returned.
   * </pre>
   *
   * <code>.google.protobuf.StringValue site = 1;</code>
   * @return The site.
   */
  com.google.protobuf.StringValue getSite();
  /**
   * <pre>
   * The domain name of the site. If the customer requesting the ideas doesn't
   * own the site provided only public information is returned.
   * </pre>
   *
   * <code>.google.protobuf.StringValue site = 1;</code>
   */
  com.google.protobuf.StringValueOrBuilder getSiteOrBuilder();
}
