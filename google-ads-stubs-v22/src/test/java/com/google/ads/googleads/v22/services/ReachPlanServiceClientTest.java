...
public class ReachPlanServiceClientTest {
  ...
  public void listPlannableLocationsTest() throws Exception {
    ...
    Assert.assertEquals(request.getReachApplicationInfo(), actualRequest.getReachApplicationInfo());
    Assert.assertTrue(
        channelProvider.isHeaderSent(
            ApiClientHeaderProvider.getDefaultApiClientHeaderKey(),
            GaxGrpcProperties.getDefaultApiClientHeaderPattern()));
  ...
  @Test
  public void listPlannableUserListsTest() throws Exception {
    ListPlannableUserListsResponse expectedResponse =
        ListPlannableUserListsResponse.newBuilder()
            .addAllPlannableUserLists(new ArrayList<PlannableUserList>())
            .build();
    mockReachPlanService.addResponse(expectedResponse);

    String customerId = "customerId-1581184615";

    ListPlannableUserListsResponse actualResponse = client.listPlannableUserLists(customerId);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<AbstractMessage> actualRequests = mockReachPlanService.getRequests();
    Assert.assertEquals(1, actualRequests.size());
    ListPlannableUserListsRequest actualRequest =
        ((ListPlannableUserListsRequest) actualRequests.get(0));
  ...
  @Test
  public void listPlannableUserListsExceptionTest() throws Exception {
    StatusRuntimeException exception = new StatusRuntimeException(io.grpc.Status.INVALID_ARGUMENT);
    mockReachPlanService.addException(exception);

    try {
      String customerId = "customerId-1581184615";
      client.listPlannableUserLists(customerId);
      Assert.fail("No exception raised");
    } catch (InvalidArgumentException e) {
      // Expected exception.
    }
  ...
  @Test
  public void listPlannableUserInterestsTest() throws Exception {
    ListPlannableUserInterestsResponse expectedResponse =
        ListPlannableUserInterestsResponse.newBuilder()
            .addAllPlannableUserInterests(new ArrayList<PlannableUserInterest>())
            .build();
    mockReachPlanService.addResponse(expectedResponse);

    String customerId = "customerId-1581184615";

    ListPlannableUserInterestsResponse actualResponse =
        client.listPlannableUserInterests(customerId);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<AbstractMessage> actualRequests = mockReachPlanService.getRequests();
    Assert.assertEquals(1, actualRequests.size());
    ListPlannableUserInterestsRequest actualRequest =
        ((ListPlannableUserInterestsRequest) actualRequests.get(0));
  ...
  @Test
  public void listPlannableUserInterestsExceptionTest() throws Exception {
    StatusRuntimeException exception = new StatusRuntimeException(io.grpc.Status.INVALID_ARGUMENT);
    mockReachPlanService.addException(exception);
